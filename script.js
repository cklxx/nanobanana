...
const IMAGE_URL = 'https://aihubmix.com/v1/images/generate';
...
const promptTemplates = [
  {
    ...
    tag: 'PPT',
    body:
      '使用玻璃拟态和渐变氛围光，留出标题与副标题区域。色彩采用深空蓝 + 品牌色点缀，光影柔和，保持极简和专业感，适合科技演讲封面。'
  },
  {
    title: '商业计划书单页',
    tag: 'PPT',
    body:
      '横版 16:9，蓝紫色系，左侧大标题，右侧视觉焦点。叠加粒子、流线和玻璃卡片，突出“增长”、“全球化”的关键词，整体高端稳重。'
  },
  {
    title: '电商极简主图',
    tag: '电商',
    body:
      '4K 产品主图，柔和顶光，背景为轻薄渐变或大理石质感，产品置于中央带镜面反射。要求无水印、高对比度，显得高级、干净。'
  },
  {
    title: '电商情绪氛围图',
    tag: '电商',
    body:
      '突出生活方式，包含软光、浅景深、环境氛围。画面带有生活化摆件和暖色侧光，突出产品使用场景，情绪感强，细节清晰。'
  },
  {
    title: '治愈系插画海报',
    tag: '插画',
    body:
      '轻柔配色、手绘笔触，角色表情温暖。包含光晕和柔焦背景，构图居中，细节丰富但保持留白，适合作为海报或社交媒体头图。'
  },
  {
    title: '会议速记 / 纪要封面',
    tag: 'PPT',
    body:
      '深色底、蓝绿色渐变流线，右上角装饰点阵。留出大标题与日期区块，质感稳重，适合企业会议纪要封面，字体建议无衬线风格。'
  }
];
...
function renderPromptLibrary() {
  ...
  promptTemplates.forEach((item, index) => {
    ...
    card.innerHTML = `
      ...
      <h3>${item.title}</h3>
      <p>${item.body}</p>
      <div class="caption">点击自动填入生成表单</div>
    `;
...
function flash(node) {
  node.animate(
    [
      { transform: 'translateY(-2px)', borderColor: 'rgba(0,245,160,0.6)' },
...
async function fetchQuota() {
  ...
  if (!key) {
    setStatus(quotaStatus, '请先填入 API Key');
    return;
  }
...
async function generateImage(event) {
  ...
  const mode = modeSelect?.value || 'text';
  const model = document.getElementById('model').value || 'imagen-3.0-generate-001';
  const size = document.getElementById('size').value;
  const style = document.getElementById('style').value;
  const removeWatermark = document.getElementById('watermark').checked;
  const seed = document.getElementById('seed').value;
  const quality = document.getElementById('quality').value;

  const payload = {
    model,
    prompt: `${prompt}\n风格: ${style}\n分辨率: ${size}\n水印: ${removeWatermark ? '关闭' : '开启'}\n清晰度: ${quality}`,
    size,
    n: 1,
    response_format: 'url',
  ...
  try {
    if (mode === 'img2img') {
      ...
      payload.reference_images = await Promise.all(files.map((file) => fileToBase64(file)));
      payload.prompt = `${payload.prompt}\n模式: 多图参考 (${payload.reference_images.length} 张)`;
    } else {
      payload.prompt = `${payload.prompt}\n模式: 文生图`;
    }
  } catch (error) {
  ...
  setStatus(generationStatus, '生成中，请稍候...');
  ...
  try {
    ...
    appendResult(imageUrl, payload.prompt);
    setStatus(generationStatus, '生成完成');
    setPill(generationStatusPill, '生成完成', 'success');
    fetchQuota();
  } catch (error) {
    console.error(error);
...
function appendResult(url, title) {
  ...
  resultCount.textContent = `${current} 张图片`;
}
...
function clearKey() {
  ...
  apiKeyInput.value = '';
  setPill(quotaPill, '额度未查询', '');
  setStatus(quotaStatus, '已清除本地 Key');
}
...
function init() {
  loadStoredKey();
  setupEvents();
  setPill(generationStatusPill, '等待输入', 'info');
